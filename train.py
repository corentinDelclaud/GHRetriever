...
def main(args):
    ...
    n_train = max(1, int(len(train_dataset)))
    train_dataset = train_dataset[:n_train]
    val_dataset = [dataset[i] for i in idx_split['val']]
    n_val = max(1, int(len(val_dataset)))
    val_dataset = val_dataset[:n_val]
    test_dataset = [dataset[i] for i in idx_split['test']]
    n_test = max(1, int(len(test_dataset)))
    ...
    best_epoch = -1  # Initialize to invalid value
    ...
    for epoch in range(args.num_epochs):
        ...
        if isinstance(epoch_loss, float) and (epoch_loss != epoch_loss):  # nan check
            print(f"Warning: Train loss is nan at epoch {epoch}. Check your data, loss function, or try lowering the learning rate.")
        if isinstance(val_loss, float) and (val_loss != val_loss):  # nan check
            print(f"Warning: Validation loss is nan at epoch {epoch}. Check your data, loss function, or try lowering the learning rate.")
        ...
        if best_epoch == -1:
            print(f"Warning: No best epoch found (val_loss always nan or never improved).")
