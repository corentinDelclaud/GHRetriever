...
class GraphLLM(torch.nn.Module):
    ...
    def forward(self, samples):
        ...
        #batch_size = len(samples['id']) if 'id' in samples else samples['graph'].num_graphs
        #print(f"batch_size: {batch_size}, graph_embeds.shape: {graph_embeds.shape}")
        #assert graph_embeds.shape[0] == batch_size, f"graph_embeds.shape[0] ({graph_embeds.shape[0]}) != batch_size ({batch_size})"
